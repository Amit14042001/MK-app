// ============================================================
// Slot Professional App - AuthContext
// ============================================================
...
export function ProfAuthProvider({ children }) {
  ...
  const restoreSession = async () => {
    try {
      const [token, profData] = await Promise.all([
        AsyncStorage.getItem('slot_prof_access_token'),
        AsyncStorage.getItem('slot_prof_data'),
      ]);
      if (token && profData) {
  ...
  const login = async (userData, accessToken, refreshToken) => {
    ...
    await AsyncStorage.multiSet([
      ['slot_prof_access_token', accessToken],
      ['slot_prof_refresh_token', refreshToken],
      ['slot_prof_data', JSON.stringify(userData)],
    ]);
  ...
  const logout = async () => {
    ...
    await AsyncStorage.multiRemove(['slot_prof_access_token', 'slot_prof_refresh_token', 'slot_prof_data']);
