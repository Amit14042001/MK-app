...
const css = `
  ...
  * { box-sizing:border-box; }
  ...
  .time-chip:hover:not(.selected) { border-color:var(--color-brand) !important; color:var(--color-brand) !important; }
  .tab-btn:hover { color:var(--color-brand) !important; }
  ...
  return (
    <div onClick={onClick} className="sub-row" style={{
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, flex: 1 }}>
        <div style={{
          width: 22, height: 22, borderRadius: '50%', border: `2px solid ${selected ? 'var(--color-brand)' : '#ddd'}`,
      ...
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <div style={{ fontWeight: 800, fontSize: 16, color: selected ? 'var(--color-brand)' : '#1a1a2e' }}>₹{sub.price}</div>
        {sub.originalPrice && (
          <div style={{ fontSize: 12, color: '#bbb', textDecoration: 'line-through' }}>₹{sub.originalPrice}</div>
        )}
        {disc > 0 && (
          <div style={{
            fontSize: 10, fontWeight: 700, color: '#27ae60', background: '#e8f5e9',
            padding: '2px 6px', borderRadius: 20, marginTop: 2
          }}>{disc}% off</div>
        )}
      </div>
    </div>
...
export default function ServiceDetailPage({ serviceId, navigate }) {
  ...
  const handleAddToCart = () => {
    ...
    addToCart({
      ...
      serviceImage: service.images && service.images.length > 0 ? service.images[0] : null,
  ...
  if (loading) return (
    <div style={{
      maxWidth: 1280, margin: '40px auto', padding: '0 24px', display: 'grid',
      gridTemplateColumns: '1fr 380px', gap: 28
    }}>
      {[0, 1].map(i => <div key={i} style={{
        height: 400, background: '#fff', borderRadius: 20,
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '28px 24px' }}>
              ...
              <span key={b} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span onClick={() => i === 0 ? navigate('home') : i === 1 ? navigate('services') : null}
                  style={{
                    ...
                    color: i < arr.length - 1 ? 'var(--color-brand)' : '#333',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 28, alignItems: 'start' }}>
              ...
              <div style={{
                ...
                <div style={{ position: 'relative' }}>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
                    {service.isNew && <span style={{
                      background: 'rgba(33,150,243,0.2)', color: '#90caf9',
                      padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700
                    }}>NEW</span>}
                    {service.isPopular && <span style={{
                      background: 'rgba(76,175,80,0.2)', color: '#a5d6a7',
                      padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700
                    }}>Popular</span>}
                    {service.warranty && <span style={{
                      background: 'rgba(255,255,255,0.1)', color: 'rgba(255,255,255,0.85)',
                      padding: '3px 10px', borderRadius: 20, fontSize: 11, fontWeight: 600
                    }}>🛡️ {service.warranty}</span>}
                  </div>
                  <div style={{ display: 'flex', gap: 20, alignItems: 'flex-start' }}>
                    <div style={{ width: 80, height: 80, borderRadius: 16, overflow: 'hidden', background: 'rgba(255,255,255,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div style={{ display: 'flex', borderBottom: '1px solid #f0f0f0' }}>
                  {TABS.map((t, i) => (
                    <button key={t} onClick={() => setActiveTab(i)} className="tab-btn" style={{
                      ...
                      color: activeTab === i ? 'var(--color-brand)' : '#777',
                  ...
                  {activeTab === 0 && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                      ...
                      ['🔧', 'Pro Equipment', 'Professional-grade tools only']
                        ...
                        <div key={t} style={{
                          display: 'flex', gap: 12, padding: '14px', background: '#f8f9fa',
                        ...
                        : reviews.map((r, i) => (
                          <div key={i} style={{ borderBottom: '1px solid #f5f5f5', paddingBottom: 16, marginBottom: 16 }}>
                            <div style={{ display: 'flex', gap: 12, alignItems: 'center', marginBottom: 8 }}>
                              <div style={{
                                width: 38, height: 38, borderRadius: '50%', background: 'var(--color-brand)',
                  ...
                  {activeTab === 3 && (
                    <div>
                      {(service.faqs?.length > 0 ? service.faqs : [
                        ...
                        { question: 'Are professionals verified?', answer: 'Yes — all undergo background checks, skill tests and Slot training before joining.' },
            ...
            <div style={{ position: 'sticky', top: 80 }}>
                    ...
                    <div>
                      ...
                      {origPrice && (
                        <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 5 }}>
                          ...
                          <span style={{
                            background: '#e8f5e9', color: '#27ae60', padding: '2px 8px',
                ...
                <div style={{ padding: '0 20px 20px' }}>
                      ...
                      {dates.map((d, i) => {
                        ...
                        return (
                          <div key={i} onClick={() => setSelectedDate(val)}
                            style={{
                              minWidth: 52, padding: '10px 0', borderRadius: 12, textAlign: 'center',
                              cursor: 'pointer', flexShrink: 0, transition: 'all 0.15s',
                              border: `2px solid ${sel ? 'var(--color-brand)' : '#f0f0f0'}`,
                              background: sel ? 'var(--color-brand)' : '#fff',
                              color: sel ? '#fff' : '#333'
                            }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                      {TIMES.map(t => (
                        <div key={t} onClick={() => setSelectedTime(t)} className={`time-chip ${selectedTime === t ? 'selected' : ''}`}
                          style={{
                            padding: '7px 10px', borderRadius: 8, cursor: 'pointer', fontSize: 11, fontWeight: 600,
                            border: `1.5px solid ${selectedTime === t ? 'var(--color-brand)' : '#f0f0f0'}`,
                            background: selectedTime === t ? 'var(--color-brand)' : '#fff',
                  ...
                  <div style={{ marginTop: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <button onClick={handleBookNow} className="action-primary"
                      style={{
                        width: '100%', padding: '15px', background: 'linear-gradient(135deg,var(--color-brand),#c0392b)',
                    ...
                    <button onClick={handleAddToCart} className="action-secondary"
                      style={{
                        width: '100%', padding: '13px', background: '#fff', color: 'var(--color-brand)',
                        border: '2px solid var(--color-brand)', borderRadius: 12, cursor: 'pointer',
              ...
              <div style={{
                background: '#fff', borderRadius: 16, padding: '16px 18px', marginTop: 16,
                border: '1px solid #f0f0f0'
              }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {['AC Service', 'Electrician', 'Plumber', 'Cleaning'].filter(s => s !== service.name).slice(0, 3).map(s => (
                    <button key={s} onClick={() => navigate('services')}
                      style={{
                        padding: '6px 12px', border: '1px solid #f0f0f0', borderRadius: 20,
